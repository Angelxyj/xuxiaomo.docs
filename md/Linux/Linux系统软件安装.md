...
在前面，我们学习了许多的Linux命令和高级技巧，这些知识点比较零散，并没有一些具体的实操能够串联起来这些知识点。

所以，各类软件在Linux上部署安装的实战非常有必要：

- 本文中演示部署的软件，仅涉及到安装部署，不对软件的使用做详细说明，使用说明有空再写。

## 前置要求

本文中会开启多台虚拟机，请尽量确保电脑的内存在：8GB（包含8GB）以上。如内存不足可以扩充内存条或购买阿里云、UCloud等云服务器临时使用（1个月多台低配服务器几十块左右）
...
4. [扩展]，配置root的简单密码
   ...
   # 然后就可以用简单密码了（本文中使用简单密码，为了方便，生产中不要这样）
...
> 本文中配置的WSL环境是最新的Ubuntu22.04版本，这个版本的软件商店内置的MySQL是8.0版本
>
...
5. 安装MySQL5.7
   ...
   弹出框中输入root密码并选择ok，密码任意，本文中以123456代替
...
> 注意，本文是以WSL获得的Ubuntu操作系统环境。
>
> 如果你通过VMware虚拟机的方式获得了Ubuntu操作系统环境，操作步骤不用担心，和本文中使用WSL环境是==完全一致的==
...
1. 首先，放行tomcat需要使用的8080端口的外部访问权限
   ...
   >
   > 防火墙的配置非常复杂，后面会视情况独立出一篇防火墙配置规则的博文。
...
RabbitMQ在企业开发中十分常见，本文为大家演示快速搭建RabbitMQ环境。
...
SELinux的配置同防火墙一样，非常复杂，文中不多涉及，后续视情况可以出一章SELinux的配置博文。
...
Kafka多数都是运行在分布式（集群化）模式下，所以文中将以3台服务器，来完成Kafka集群的安装部署。
...
# 然后就可以用简单密码了（文中使用简单密码，为了方便，生产中不要这样）
...
大多数情况下, `Grafana`都是部署在`linux`服务器之上. 所以本文也是基于`Linux`系统来讲解.
...
我们使用`Mysql`. 和`zabbix`的元数据mysql共用一个实例
